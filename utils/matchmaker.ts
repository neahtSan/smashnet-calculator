...
export const findFirstMatch = (players: Player[]): [Player, Player, Player, Player] => {
  ...
  const group1 = players.slice(0, 4);
  const group2 = players.slice(4);
...
export const findSecondMatch = (players: Player[], previousMatch: Match): [Player, Player, Player, Player] => {
  ...
  const selectedPlayers = getRandomPlayers(losingTeam.filter(p => p !== losingTeamPlayer), 3, baseSeed + 1);
...
export const findBestMatch = (players: Player[], previousMatches: Match[]): [Player, Player, Player, Player] => {
  ...
  const team1Player1 = getRandomPlayer(sortedPlayers, baseSeed);
  const team2Player1 = getRandomPlayer(sortedPlayers.filter(p => p !== team1Player1), baseSeed + 1);
